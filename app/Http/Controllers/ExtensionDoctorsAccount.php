...
class ExtensionDoctorsAccount extends Controller
{
    /*
    *
    ...
    */
    ...
    private $Folder = 'extension'; //Set Default Folder For html files
    private $SubFolder = '/accounts/doctors'; //Set Default Sub Folder For html files and Front End Use Start with /

    private $AllowedFile = null; //Set Default allowed file extension, remember you can pass this upon upload to override default allowed file type. Allowed File Extensions Separated by | also leave null to validate using jpg|jpeg|png|doc|docx|pdf|xls|txt change this on validation function at the bottom

    private $Route = 'doctors'; //If you have different route Name to Module name State it here |This wont be pluralized | set it null to use default

    private $New = 'doctors/open/add'; //New User
    private $Save = 'doctors/valid/save'; //Add New User
    private $Edit = 'doctors/valid/update'; //Update User

    private $ModuleName = 'Doctors'; //Module Nmae
    ...
    */
    public function index($notifyMessage=null)
    {
        ...
        //Table Select & Clause
        $where = array('level' => 'doctor');
        $columns = array('id,level as level,logname as phone_number,name as full_name,email as email,flg as status');
    ...
    */
    public function edit($pageID,$inputTYPE='id',$inputID=null,$message=null,$layout='extend')
    {
        ...
        if (!is_null($inputTYPE) || !is_null($inputID)) {
            ...
            $columns = array('id as id,name as name,email as email,level as level,logname as logname,details as details');
    ...
    public function valid($type)
    {
        ...
        if ($type == 'save') {
            ...
            $validatedData = Validator::make($formData,[
                ...
                'user_mobile' => ['required','min:1','unique:users,user_logname'],
                'user_password' => ['required','min:4'],
                'doctor_licence' => ['required','min:4'],
            ])->validate();;

            //Form Validation
            if ($validatedData == TRUE) {

            	$formData['user_logname'] = $formData['user_mobile'];
            	$formData['user_level'] = 'doctor';

                if ($this->create($formData,array('user_mobile'))) {
        ...
        elseif ($type == 'update') {
            ...
            $validatedData = Validator::make($updateData,[
                ...
                'user_mobile' => ['required','min:1','max:100',$this->LognameCheck],
            ...
            if (array_key_exists("$column_password",$updateData)) {
                if (!empty(Input::post($column_password))) {	$unsetData= array('id','user_mobile','doctor_licence');/*valude To Unset */}
                else{ $unsetData= array('id',$column_password,'user_mobile','doctor_licence');/*Unset Value*/	}
            }else{$unsetData = array('id','user_mobile','doctor_licence');/*value To Unset*/}
            ...
            if ($validatedData == TRUE) {
            	...
            	$updateData['user_logname'] = $updateData['user_mobile'];
